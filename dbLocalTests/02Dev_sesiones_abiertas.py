...
def open_sessions_PostgreSQL():
    ...
    try:
        ...
        # execute a statement
        cur.execute(
            f"SELECT row_to_json(t) FROM (SELECT * FROM pg_stat_activity WHERE datname='{dbConfigLocal['database']}') t")
        ...
        for activity in all_activity:
            ...
            print('   ', 'Fecha Local del Query:', parser.isoparse(
                act['query_start']).astimezone(tz.tzlocal()))
            i += 1
